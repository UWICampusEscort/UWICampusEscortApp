...
const EscortTravelGroup = ({ data, current_user }: { data: TravelGroupData, current_user: string }) => {
  ...
  return (
    <Card className="mb-6 w-full h-fit">
      ...
      <CardContent className="flex justify-between items-center gap-4">
        {current_user && data.escorts.includes(current_user) ?
          <Button variant={"secondary"} size={"sm"} className="w-full" disabled>
            Already Escorting
          </Button>
          : (data.rejected_escorts.includes(current_user) ?
            <Button variant={"destructive"} size={"sm"} className="w-full" disabled>
              Rejected from Group
            </Button>
            :
            <Button variant={"secondary"} size={"sm"} className="w-full" onClick={escortGroup} disabled={current_user === data.created_by || has_departed(data.departure_time)}>
              Escort Group{data.requested_escorts.includes(current_user) ? " (Requested)" : ""}
            </Button>
          )
        }
                    ...
                    <div className="h-48 overflow-y-auto overflow-x-hidden border inset border-border rounded-md p-1 scrollbar-thin">
                      {Array.from(new Set([...data.escorts, ...data.requested_escorts])).map((escortId) => {
                        return <div key={escortId} className="flex items-center gap-2 p-2">
                          ...
                          {data.escorts.includes(escortId) &&
...
export default function HomePage() {
  ...
  const CreateGroup = () => {
    return <Dialog open={isCreateGroupOpen} onOpenChange={setIsCreateGroupOpen}>
      <DialogTrigger asChild>
          ...
          <FieldGroup>
            ...
            {requestingEscort && <>
              ...
              <div className="flex gap-2 items-center">
                <RequestEscort escortUsers={availableEscorts.filter(user => user.id !== userId)} specificEscorts={specificEscorts} setSpecificEscorts={setSpecificEscorts} />
              </div>
            </>}
  ...
  const EscortGroup = () => {
    return <Dialog open={isEscortGroupOpen} onOpenChange={setIsEscortGroupOpen}>
      <DialogTrigger asChild>
      ...
      <DialogContent className="min-w-[75vw] max-h-[85vh] overflow-x-hidden overflow-y-auto scrollbar-thin">
        ...
        <div className="flex flex-col gap-2 w-full h-[60vh] overflow-y-auto scrollbar-thin px-4 py-4">
          {groups.filter(group => group.alive && group.need_escort && (group.requested_escorts.length == 0 || group.requested_escorts.includes(userId))).sort((a, b) => new Date(a.departure_time).getTime() - new Date(b.departure_time).getTime()).map((group) => (has_departed(group.departure_time) ? null :
            <EscortTravelGroup key={group.id} data={group} current_user={userId} />
          ))}
        </div>
    ...
    <div className="mx-auto w-full max-w-5xl px-4 sm:px-6 lg:px-8 pt-8">
      <div className="w-full flex justify-between items-center">
        ...
        <div className="flex gap-2 items-center">
          ...
          <div className="flex flex-col sm:flex-row gap-2">
            <CreateGroup />
            {availableEscorts.find(user => user.id === userId) &&
              <EscortGroup />
            }
          </div>
        </div>
